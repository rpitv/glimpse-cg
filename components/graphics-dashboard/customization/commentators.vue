<template>
  <div>
    <UFormField label="Offset X" help="Adjust the horizontal position of the commentators. Positive values move it to the right, negative values to the left.">
      <UInputNumber v-model="commentators.offsetX" :step="0.1" :format-options="{ minimumFractionDigits: 1 }" />
    </UFormField>
    <UFormField class="mt-4" label="Offset Y" help="Adjust the vertical position of the commentators. Positive values move it up, negative values down.">
      <UInputNumber v-model="commentators.offsetY" :step="0.1" :format-options="{ minimumFractionDigits: 1 }" />
    </UFormField>
    ...
    <div class="mt-2">
      ...
      <p v-show="false">{{ commentatorsRef }}</p>
      <table class="w-full">
        <thead>
          <tr>
            <th>Commentator</th>
          ...
          <tr v-for="(commentator, index) in commentatorsRef" :key="index">
            <td>
              <div class="w-full">
                <UFormField label="Name" help="The name of the commentator.">
                  <UInput v-model="commentatorsRef[index]!.name" placeholder="Name" class="w-full" />
                </UFormField>
                <UPopover arrow>
                  <UButton
                    class="mt-2"
                  ...
                  <template #content>
                    <div class="p-2" style="box-shadow: 0 4px 16px rgba(0,0,0,0.9); border-radius: 0.5rem;">
                      <ColorPicker v-model="commentatorsRef[index]!.nameColor" label="Name Color" help="The color for the commentator's name." />
                      <UFormField class="mt-4" label="Name Size" help="Adjust the size of the commentator's name.">
                        <UInputNumber v-model="commentatorsRef[index]!.nameSize" :step="0.1" :format-options="{ minimumFractionDigits: 1 }" />
                      </UFormField>
                    </div>
                  </template>
                </UPopover>
            ...
            <td>
              <div class="flex gap-2 justify-center">
                <UButton size="sm" color="error" @click="deleteCommentator(index)">Delete</UButton>
              </div>
            </td>
          </tr>
...
let sortableInstance = useSortable(".my-table-tbody", commentatorsRef, { animation: 150 });
