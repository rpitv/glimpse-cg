  ...
  <UCard class="rounded-none">
    ...
    <template #default>
      ...
      <div class="mt-4 flex justify-center">
        <UTooltip text="Controls cannot be edited when sync is enabled." :disabled="!sync.clock">
          <UFieldGroup>
            <UInput
              :disabled="sync.clock"
              placeholder="MM:SS.s"
            ...
            />
            <UButton :disabled="sync.clock" @click="setTime()">Set Time</UButton>
          </UFieldGroup>
        </UTooltip>
      </div>
      <UTooltip text="Controls cannot be edited when sync is enabled." :disabled="!sync.clock">
        <UFieldGroup class="mt-4 flex justify-center w-full" size="xl">
          <UButton variant="outline" @click="clock.time = Math.max(0, clock.time - 1000)" :disabled="sync.clock">
            <FontAwesomeIcon icon="fas fa-minus" />1s
          </UButton>
          <UButton variant="outline" @click="clock.time = Math.max(0, clock.time - 100)" :disabled="sync.clock">
            <FontAwesomeIcon icon="fas fa-minus" />0.1s
          </UButton>
          <UButton variant="subtle" @click="clock.isRunning = !clock.isRunning" :disabled="clock.time === 0 || sync.clock">
          ...
          <UButton variant="outline" @click="clock.time += 100" :disabled="sync.clock">
            <FontAwesomeIcon icon="fas fa-plus" />0.1s
          </UButton>
          <UButton variant="outline" @click="clock.time += 1000" :disabled="sync.clock">
            <FontAwesomeIcon icon="fas fa-plus" />1s
          </UButton>
          <UButton variant="outline" color="secondary" @click="clock.time = 120_000; clock.isRunning = false" :disabled="sync.clock">
